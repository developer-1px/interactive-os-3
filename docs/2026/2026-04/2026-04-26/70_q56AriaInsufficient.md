...
`packages/ds/src/core/gesture.ts`가 raw event → semantic event 변환 자리다. drag preview의 위치/내용은 `useResource` 상태로 직렬화되고(C2), preview entity가 그 상태를 `data` prop으로 받아 그린다. ARIA가 부족한 게 아니라, 부족한 영역은 entity로 분리해 그 안에서 ARIA-적합하게 다시 표현하는 것이다.

## 근거

- `/Users/user/Desktop/ds/CANONICAL.md:39-40` — gesture/intent 분리, activate 단발
- `/Users/user/Desktop/ds/packages/ds/src/core/gesture.ts` — raw → semantic 변환 자리
