...
- **상태 층** — `value`. JSON 가능해야 한다. `packages/ds/src/data.ts:138` `getEntry(key).value`가 store에 저장되는 값으로 직렬화 round-trip 대상.
- **이벤트 층** — `dispatch(e)`의 `e: ResourceEvent<T>`. `{ type:'set', value }`, `{ type:'patch', partial }`, `{ type:'refetch' }`, `{ type:'invalidate' }` — 모두 데이터 객체다 (`packages/ds/src/data.ts:13-19`). 이게 직렬화 가능해야 한다. **dispatch 함수 자체는 채널이지 상태가 아니다**.
...
- `/Users/user/Desktop/ds/packages/ds/src/data.ts:13-19` — ResourceEvent = plain discriminated union
- `/Users/user/Desktop/ds/packages/ds/src/data.ts:142-160` — dispatch가 받는 `e`만이 핵심. `entry.value`만 store
- `/Users/user/Desktop/ds/packages/ds/src/core/flow.ts:43,49-58` — useFlow도 동일 패턴: value 직렬화 + event(plain) 흐름
