---
id: devicesConceptTree
type: inbox
slug: devicesConceptTree
title: ds 장치 컨셉맵 — ASCII Tree
tags: [inbox, explain, vision]
created: 2026-04-26
updated: 2026-04-26
---

# ds 장치 컨셉맵 — ASCII Tree

## 배경

ds가 원칙(decision-shrinking, classless, data-driven, 직렬화)을 강제하기 위해 **실제로 코드에 박아둔 장치(mechanism)** 를 한 그림으로 정리한다. 원칙은 추상이지만 장치는 파일·함수·hook·lint 룰로 존재한다.
...
3. **각 장치는 단일 책임** — `data-part`는 부품 namespace 1개 책임, `defineFeature`는 spec 1개 책임. SRP 위반 시 (예: useMemo) 신호로 봄.
4. **어휘 → lint** 1:1 매핑이 핵심. 정본 1줄이 lint 1개로 직결되지 않으면 정본은 종이 위에서만 산다.

## 다음 행동

- 이 트리를 `CANONICAL.md` "장치 인덱스" 섹션으로 흡수 검토
- 각 lint script가 어떤 정본 줄을 검증하는지 1:1 매핑 표 작성 (Q43 답변 보강)
- ui/0-primitive ~ 8-layout 번호 체계 자체가 위계 토큰 — 별도 explain 문서 후보
