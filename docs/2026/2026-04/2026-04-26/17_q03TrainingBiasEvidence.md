...
ds의 "Radix·Ariakit·RAC·Base 중 2곳 이상 수렴" 기준 (CANONICAL.md "네이밍·구조" 섹션, src/ds/ 컴포넌트 명명)은 사실 학습 분포에 대한 **간접 베팅**이다. 이 라이브러리들은 npm download가 많고 GitHub stars가 많다 → 학습 corpus에 자주 등장했을 확률이 높다 → 모델이 그 어휘를 안정적으로 호출한다. 이건 단언이 아니라 **휴리스틱**이다. de facto 라이브러리 4개라는 숫자도 임의(arbitrary)다 — Headless UI·Reach UI는 왜 빠졌나 (Q60)에 깔끔한 답이 없다.
