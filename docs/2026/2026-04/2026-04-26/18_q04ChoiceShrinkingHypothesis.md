...
packages/ds/src/ui/2-button-link/ 에 SubmitAction·DangerAction·LinkAction을 분리한 후, 코드 작성을 모델에 시킬 때 "어떤 버튼?"을 두 번 묻는 일이 사라졌다. variant 시절엔 매번 prop 조합을 추정했는데, role 분리 후엔 import 이름만 정해진다. 이게 N=수십 정도의 비공식 관찰이다 — 통계가 아니다.
...
- `/Users/user/Desktop/ds/packages/ds/src/ui/2-button-link/` — role 분리 실증
