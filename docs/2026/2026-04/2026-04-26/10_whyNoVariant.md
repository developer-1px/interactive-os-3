...
type: inbox
slug: whyNoVariant
title: Why No Variant — variant prop을 금지하는 이유
tags: [inbox, vision, explain]
created: 2026-04-26
updated: 2026-04-26
---

# Why No Variant — variant prop을 금지하는 이유
...
variant는 이 셋을 묶어서 1개 enum으로 압축한다. 압축 결과는 **소비자가 매 호출마다 다시 풀어야 하는** 묶음이다. 어떤 호출에서는 색만 바꾸고 싶고, 어떤 호출에서는 의미만 바꾸고 싶고, 어떤 호출에서는 위치만 바꾸고 싶다 — 그러면 또 prop을 추가하게 된다 (`color`, `emphasis`, `placement`...). 폭증의 시작이다.
...
#### Q1. size는?
A. 대부분의 size는 surface 위계에 흡수된다. atom < section < surface < shell 5단 hierarchy 토큰이 size를 결정한다. 컴포넌트가 자기 size prop을 가지지 않는다.

#### Q2. responsive는?
A. CSS only. JS variant 분기 금지 (`<Button variant={isMobile ? 'compact' : 'full'}>` 금지). 컴포넌트는 viewport 무관하게 1개 형태고, CSS가 reflow·축소를 담당한다.

#### Q3. theme(dark/light)은?
A. variant가 아닌 color token tier로 흡수. semantic token이 자동으로 mode를 따라간다. 컴포넌트는 mode를 모른다.

#### Q4. 같은 buton인데 32곳에서 다른 padding이 필요하면?
A. 32곳이 다른 padding이라면 32곳의 surface가 다른 hierarchy level이라는 뜻이다. button이 아닌 surface를 정정한다. 그래도 안 풀리면 surface를 새로 만든다.

### 7. variant를 허용했을 때의 미끄러짐
...
- `whyDataDriven.md`: data prop·children 금지의 why
- 컴포넌트 어휘 사전을 별도 reference 문서로 (현재는 ds/ui/* 폴더 구조에 암묵적으로 존재)
