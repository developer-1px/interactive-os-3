...
title: "@p/aria-kernel 사용 가이드 — 외부 프로젝트 통합"
tags: [inbox, explain, vision]
created: 2026-04-30
updated: 2026-04-30
---

# @p/aria-kernel 사용 가이드 — 외부 프로젝트 통합

## 배경

`@p/aria-kernel`는 ds 모노레포에서 갓 분리된 단일 패키지로, ARIA 행동 인프라(axes 합성 · roving tabindex · gesture/intent 변환 · FlatLayout 선언적 트리 · Resource 단일 데이터 인터페이스 · feature/middleware)를 모은다. 컴포넌트 어휘(tokens·CSS·UI primitive)에 의존하지 않으므로 다른 프로젝트에서 독립 사용 가능하다. 본 문서는 외부 consumer가 채택할 때의 설치·의존성·API 활용·통합 패턴을 정리한다.
...
```
@p/aria-kernel = 단일 npm 패키지 (Base UI / Ariakit 식)
```
...
resolve: {
  alias: [
    { find: /^@p\/headless$/,
      replacement: resolve(__dirname, 'packages/aria-kernel/src/index.ts') },
    { find: /^@p\/headless\//,
      replacement: resolve(__dirname, 'packages/aria-kernel/src/') + '/' },
  ],
}
```
...
// tsconfig.json
"paths": {
  "@p/aria-kernel": ["packages/aria-kernel/src/index.ts"],
  "@p/aria-kernel/*": ["packages/aria-kernel/src/*"]
}
```
...
```ts
import { type NormalizedData, ROOT } from '@p/aria-kernel'
...
```tsx
import { ROOT, useRovingTabIndex, composeAxes, navigate, activate } from '@p/aria-kernel'
...
```ts
import { composeGestures, navigateOnActivate, selectionFollowsFocus, type UiEvent } from '@p/aria-kernel'
...
```tsx
import { useControlState, fromList } from '@p/aria-kernel'
...
```ts
import { defineResource, useResource } from '@p/aria-kernel'
...
```ts
import { defineFeature, useFeature } from '@p/aria-kernel'
...
```tsx
import { definePage, ROOT } from '@p/aria-kernel'
...
```ts
import { defineLayout, defineWidget, merge } from '@p/aria-kernel'
...
```ts
import { defineMiddleware } from '@p/aria-kernel'
...
`@p/aria-kernel/layout`은 *선언* 어휘만 제공한다. 실제 렌더링은 consumer가:

```tsx
import { useDebugTree, type NormalizedData } from '@p/aria-kernel'
...
`packages/aria-kernel/src/INVARIANTS.md`에 전문. 핵심:
...
### 7. 차별점 — 왜 @p/aria-kernel인가

| 비교 | Radix Primitives | Ariakit | React Aria | **@p/aria-kernel** |
|---|---|---|---|---|
...
### 9. 마이그레이션 체크리스트 (Radix → @p/aria-kernel)
