...
type: inbox
slug: headlessUsage
title: "@p/aria-kernel usage — 실제 API 기반 사용법"
tags: [inbox, explain]
created: 2026-04-30
updated: 2026-04-30
---
...
소스 파일을 직접 읽고 정정한 사용 가이드. 모든 시그니처는 `packages/aria-kernel/src/`에서 검증됨.
...
// package.json
{
  "dependencies": { "@p/aria-kernel": "workspace:*" },
  "peerDependencies": { "react": "^19.0.0" }
}
```

`@p/aria-kernel`만 받으면 충분하다. tokens·CSS·UI 어휘 0건.
...
```ts
import { activateProps } from '@p/aria-kernel'
...
```tsx
import { useControlState, fromTree } from '@p/aria-kernel'
...
```ts
import { defineFlow, useFlow } from '@p/aria-kernel'
...
쿼리 무효화:
```ts
import { invalidateQuery } from '@p/aria-kernel'
...
```ts
import type { CollectionProps, ControlProps } from '@p/aria-kernel'
...
`@p/aria-kernel/layout`은 *선언* 어휘만 제공. 렌더링은 consumer가 담당.
