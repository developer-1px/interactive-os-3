...
title: "@p/headless × aria-design-system 통합 — ARIA만으로 동작 만들기"
tags: [inbox, explain, vision]
created: 2026-04-30
updated: 2026-04-30
---

# @p/headless × aria-design-system 통합
...
const { focusId, bindFocus, delegate } = useRoving(tabsBehavior.axis, data, onEvent)
...
npm install @p/headless@workspace:*
...
// tsconfig.json — paths
"paths": {
  "@p/headless": ["../ds/packages/headless/src/index.ts"],
  "@p/headless/*": ["../ds/packages/headless/src/*"]
}
```
...
resolve: {
  alias: [
    { find: /^@p\/headless$/,
      replacement: resolve(__dirname, '../ds/packages/headless/src/index.ts') },
    { find: /^@p\/headless\//,
      replacement: resolve(__dirname, '../ds/packages/headless/src/') + '/' },
  ],
}
```
...
import { composeAxes, navigate, activate } from '@p/headless'
import { composeGestures, activateOnNavigate } from '@p/headless'
export const axis = composeAxes(navigate('horizontal'), activate)
export const gesture = composeGestures(activateOnNavigate)
```
...
```ts
import { composeAxes, treeNavigate, treeExpand, activate } from '@p/headless'
...
```ts
// Grid — useRovingDOM (composition 기반)
...
```tsx
import { useControlState, useRoving, fromList, ROOT, type Event } from '@p/headless'
import { tabsBehavior } from './Tabs.behavior'

function Tabs({ items }: TabsProps) {
  const initial = fromList(items, {
    getId:  i => i.id,
...
- state 선언이 ARIA 어휘 (NormalizedData + Event) 직접 사용
...
```ts
import { fromList, ROOT } from '@p/headless'

const initial = fromList<TabItem>(items, {
  getId:  i => i.id,
...
```ts
import { defineFlow } from '@p/headless'

const tabsFlow = defineFlow({
  source: tabsResource,                    // URL/store/in-memory
  base: (value) => fromList(value?.items ?? [], { ... }),
  gestures: composeGestures(activateOnNavigate),  // selection-follows-focus 등
})
...
5. **Grid / TreeGrid** — 2D, useRovingDOM 사용
...
> ARIA `role` family → `useRoving` (data) 또는 `useRovingDOM` (composition).
