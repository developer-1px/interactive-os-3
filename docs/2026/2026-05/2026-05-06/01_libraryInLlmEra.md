---
title: LLM 시대에 라이브러리를 만든다는 것 — @p/headless 정당화 로그
date: 2026-05-06
type: inbox
status: raw
...
> @p/headless의 존재 이유를 흔드는 8개의 질문, 그리고 그 끝에 남은 좌표
...
답: **AI에게 "올바른 정본"을 주입하기 위해서**. `@p/headless`는 인간이 쓰는 라이브러리가 아니라, **AI가 정확하게 W3C APG를 구현하도록 만드는 정본 layer**.
...
### @p/headless의 답 (메모리 정본 기준)

| Radix 약점 | @p/headless 답 |
|---|---|
...
`@p/headless`는 "라이브러리"가 아니라 **"내가 0부터 다시 시작하지 않기 위한 정본"**.
