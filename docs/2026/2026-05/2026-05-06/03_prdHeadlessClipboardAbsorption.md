...
`@p/headless`로 productivity 앱을 만드는 사용자는 컬렉션에 쓰기(copy/cut/paste/undo/redo/delete)를 붙이려 할 때마다 매 widget에서 같은 보일러플레이트를 손으로 짠다.
...
2. **`@p/headless/adapters/zod-crud` subpath 신설 — `useZodCrudResource(resource, crud, flatten)` hook.** `[data, dispatch]` 시그니처(useResource 정합). 안에서 crud.subscribe 통합, flatten + meta.focus 합성, onEvent 핸들러가 `event.clipboardData.setData/getData` 처리 후 jsonCrud op 호출, 결과 focusNodeId를 meta.focus로 자동 반영.
...
1. As a `@p/headless` 사용자(in-house productivity 개발자), I want pattern hook 한 번 호출하면 Cmd+C/X/V/Z/Backspace가 자동 작동하기를 원한다, so that 매 widget에서 단축키 hook 두 줄을 쓰지 않아도 된다.
...
15. As a `@p/headless` 메인테이너, I want 새 인터페이스/추상을 추가하지 않기를 원한다, so that LLM 의사결정 부담이 늘지 않는다(`feedback_minimize_choices_for_llm`).
16. As a `@p/headless` 메인테이너, I want NormalizedData/UiEvent 정본을 그대로 활용하기를 원한다, so that 어휘가 두 곳으로 갈라지지 않는다.
...
NormalizedData/UiEvent/ControlProps/Meta는 이미 정본(`packages/headless/src/types.ts`). UiEvent에 `copy/cut/paste/undo/redo/insertAfter/appendChild/update/remove`이 1급 등재됨(`types.ts:69-81`). routeUiEventToCrud도 이미 존재(`store/routeUiEventToCrud.ts`). 새 인터페이스/타입/추상 추가 0건.
...
신설: `@p/headless/adapters/zod-crud` subpath. peerDependency: `zod-crud`. 코어 `@p/headless`는 zod-crud 비의존.
...
- `packages/headless/src/key/useHistoryShortcuts.ts` 삭제
- `packages/headless/src/key/useClipboardShortcuts.ts` 삭제
- `packages/headless/src/key/index.ts` 두 export 줄 삭제
- `packages/headless/src/index.ts` 두 export 줄 삭제 (`types.ts`에 등록돼 있다면)
...
- `packages/headless/src/patterns/*.test.tsx` (memory 통계 — 다수 존재했음)
...
- **헤드리스 React 첫 사례**: 컬렉션 쓰기를 1급으로 다루는 헤드리스 React 라이브러리 0개. `@p/headless`가 productivity-shaped 자리 차지.
