...
ds는 카탈로그 라우트로 비슷한 일을 한다 — `/content`·`/foundations`처럼 시연/카탈로그 라우트를 따로 둔다(이 라우트 본문에 한해 raw role도 허용된다). 즉 "사람이 컴포넌트를 한눈에 본다"는 niche는 동일하게 필요하고 ds는 그 자리를 라우트로 채울 뿐 Storybook 자체와 충돌하지 않는다.
...
- ds도 사람 검증용 카탈로그가 필요하다(`/content`·`/foundations` 등). 정본 외부에서 보면 사실상 Storybook을 다시 발명한 것이다.
