...
function FinderDesktop({ path, onNavigate }: { path: string; onNavigate: (p: string) => void }) {
  ...
  return (
    <main aria-roledescription="finder" aria-label="Finder" data-view={smart ? 'list' : view}>
      <TitleBar
        ...
        onBack={() => onNavigate(smart ? '/' : chain[chain.length - 2]?.path ?? '/')}
        view={smart ? 'list' : view}
        onViewChange={setView}
      />
      <section aria-roledescription="body">
        <Sidebar current={path} onPick={onNavigate} />
        {smart
          ? <ListView node={null} items={smartItems(smart.id)} currentPath={path} onNavigate={onNavigate} />
          : view === 'columns'
            ? <Columns chain={chain} rootPath={favoriteRoot} onNavigate={onNavigate} />
            : <ListView node={listAnchor} currentPath={path} onNavigate={onNavigate} />}
