 ...
 * 분기 여부의 의미 분해(navigate + expand vs navigate + activate)는
 * gesture 헬퍼(expandBranchOnActivate 등)가 담당.
 *
 * 키보드 Enter/Space 는 리프에서만 fire — 분기는 treeExpand 가 선점 처리.
 * disabled 는 무시.
 */
export const activate: Axis = (d, id, t) => {
  if (isDisabled(d, id)) return null
  if (t.kind === 'click') return [{ type: 'activate', id }]
  if (t.kind === 'key' && KEY_TRIGGER.has(t.key) && !getChildren(d, id).length)
