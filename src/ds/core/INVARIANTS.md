...
- [ ] 어떤 위젯이 `tabIndex={...}` 를 자식에 직접 주입하는데 `useRoving*` 을 안 쓰는가? → **1·2 드리프트 위험**
...
| 폴더 | zone | 계약 |
|---|---|---|
| `collection/` | data-driven | `CollectionProps={data, onEvent}` + `useRoving`, item 은 leaf variant 의 closed schema |
| `composite/`  | composition roving | `children: ReactNode` + `useRovingDOM`, group 단위 Tab stop |
