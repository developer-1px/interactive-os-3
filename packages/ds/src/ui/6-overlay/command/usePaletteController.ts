...
import { defineFlow, useFlow } from '../../../headless/flow'
import { expandBranchOnActivate } from '../../../headless/gesture'
...
export function usePaletteController() {
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (mode === 'columns' && e.key === 'ArrowDown') {
      const all = columnsHostRef.current?.querySelectorAll<HTMLElement>('[tabindex="0"]')
      const item = all && all[all.length - 1]
