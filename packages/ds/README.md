...
그 외 모든 CSS는 **소비처가 소유**한다 (shells / 검증 apps).
...
| **검증 앱 chrome** (sidebar shell, command palette …) | `packages/ds/src/shells/<area>/<name>.style.ts` | shells 소유 |
| **검증 앱 페이지** (board/chat/feed/inbox …) | `apps/<app>/src/{style.ts \| pages.style.ts}` | 소비 쇼케이스 소유, `main.tsx` 등록 |
...
## 검증 앱 레이어 (shells / apps)

DS 밖에서 자기 CSS 를 소유하는 단위. **3-layer**:

```
shells/<area>/<name>.style.ts     — 다중 앱 공유 chrome (sidebar shell 등)
apps/<app>/src/style.ts           — 단일 검증 앱 전역 (finderCss · inboxCss …)
apps/<app>/src/<feature>/style.ts — 검증 앱 내 페이지/feature (genrePagesCss …)
```
...
- **apps** — `apps/<app>/style.ts` 들 (검증 쇼케이스 후순위, 마지막 override)
...
| 검증 앱 페이지 스타일을 DS 에 ([data-part="board-page"] in DS) | apps/<app>/style.ts 소유 |
