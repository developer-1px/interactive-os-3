...
| `useTreeGridPattern` | [`/treegrid/`](https://www.w3.org/WAI/ARIA/apg/patterns/treegrid/) | `(data, onEvent, opts?) → { rootProps, rowProps, cellProps, items }` | `multiSelectable?: boolean`, `selectionFollowsFocus?: boolean`, `orientation?`, `autoFocus?`, `containerId?` | `treeNavigate` + `treeExpand` + `activate` (+ `multiSelect` if multi) + rowheader/gridcell semantics | tree-visible row focus with grid cells |
