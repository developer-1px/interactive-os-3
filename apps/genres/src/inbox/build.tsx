...
export function buildInboxPage(s: InboxState): NormalizedData {
  ...
  return {
    entities: {
      ...
      detailFolderTag: { id: 'detailFolderTag', data: { type: 'Ui', component: 'Badge', props: { tone: 'default', children: currentFolderLabel(s.folder) } } },
      detailTime: { id: 'detailTime', data: { type: 'Text', variant: 'small', content: cur?.time ?? '' } },
      detailLabel: { id: 'detailLabel', data: { type: 'Ui', component: 'Badge', props: { tone: cur?.label ? labelTone[cur.label] : 'default', children: cur?.label ?? '—' }, hidden: !cur?.label } },
